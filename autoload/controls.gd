...
const JOYPAD_DETECT_DEADZONE: float = 0.5
const JOYPAD_LOOK_DEADZONE: float = 0.001
...
@onready var device: String = ""
@onready var device_index: int = 0 if has_joypad() else -1


func _ready():
	process_mode = Node.PROCESS_MODE_ALWAYS
	device = guess_device_name()
	Input.joy_connection_changed.connect(
		func(device_ind, connected):
			joypad_changed.emit(device_index, is_connected)
	)
	_update_mouse_mode()
...
func _input(event: InputEvent) -> void:
	...
	if event is InputEventKey or event is InputEventMouseButton or event is InputEventMouse:
		next_device = DEVICE_KEYBOARD
		next_device_index = -1
	# Did we just use a joypad?
	elif event is InputEventJoypadButton \
		or (event is InputEventJoypadMotion and abs(event.axis_value) > JOYPAD_DETECT_DEADZONE):
...
func get_simplified_device_name(raw_name: String) -> String:
	match raw_name:
		...
		_:
			# Not using DEVICE_GENERIC so fallback button prompts are XBOX
			return DEVICE_XBOX_CONTROLLER
...
func get_label_for_input(input: InputEvent, for_device: String) -> String:
	if input == null: return ""

	if input is InputEventKey:
		return input.as_text().replace(' (Physical)','')
		...
		match for_device:
			DEVICE_XBOX_CONTROLLER, DEVICE_GENERIC:
				return XBOX_BUTTON_LABELS[input.button_index]
			DEVICE_SWITCH_CONTROLLER, DEVICE_SWITCH_JOYCON_LEFT_CONTROLLER, DEVICE_SWITCH_JOYCON_RIGHT_CONTROLLER:
				return SWITCH_BUTTON_LABELS[input.button_index]
			DEVICE_PLAYSTATION_CONTROLLER:
				return PLAYSTATION_BUTTON_LABELS[input.button_index]
...
func is_input_valid_binding(event: InputEvent) -> bool:
	var valid = true
	if event == null:
		valid = false
	if event is InputEventMouseMotion: # or event is InputEventWithModifiers:
		valid = false
...
func serialize_inputs_for_actions(actions: PackedStringArray = []) -> String:
	...
	for action in actions:
		...
		for input in inputs:
			if input is InputEventKey:
				var s: String = get_label_for_input(input, Controls.DEVICE_KEYBOARD)
...
func _update_mouse_mode():
	# update the mouse mode when the device changes
	if device == DEVICE_KEYBOARD and show_mouse:
