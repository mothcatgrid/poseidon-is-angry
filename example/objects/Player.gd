...
@onready var state_label = $Head/Camera3D/StateLabel
...
func _integrate_forces(physics_state):
	...
	if not paused:
		...
		match state:
			...
			STATE.FLOATING:
				physics_state.apply_central_force(Vector3.DOWN * 9.8)
	
				var wave = ocean.get_wave(position.x, position.z)
				...
				if height < wave_height:
					...
					physics_state.apply_central_force(Vector3(0, 9.8 * buoyancy, 0))
		...
		physics_state.apply_central_force(acc * mass)

func swap_state():
	var wave_height = ocean.get_wave(position.x, position.z).y
	var new_state = state
	match state:
		STATE.WALKING:
			# Are we in water
			if position.y < wave_height:
